...
//			var es=4;
	...
	parseText:function(text,start,context){
		...
		var match = text2.match(/^(?:([a-z]+)\:)?(?:([a-z]+|[!#]?)[\{]|([a-z]+\b)(\$)?)/i);
		try{
//			var es = 0;
			if(match){
...
//					es =2
				...
				if(ns == null){
					...
					if(fp){
						//{开始的位置，el内容
						var text3 = typeof elfn  == 'string' && text2.substring(matchLength-1) || end$;
						var seekMap = fp.seekMap;
						//try {
                            //console.log(match, text3, fn, seekMap[fn] + '')
                            if (fn in seekMap) {
                                var fnimpl = seekMap[fn];
                                //console.log(fnimpl + '')
                                var rtv = fnimpl.call(context, text3);
                                if (rtv > 0 || rtv === 0) {
                                    return start + matchLength + rtv + 1
                                }
                            } else {//if(fn){
                                console.warn("文本解析时,找不到相关的解析函数,请检查模板源码,是否手误：[code:" + match[0] + ",document:" + (context && context.currentURI) + '@[' + (node && node.lineNumber) + "]]")
                                //return -1;
                            }
                        //}catch(e){
						//	console.error("##############")
